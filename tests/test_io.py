...
def test_load_detections_concatenates_all_csvs(detections_data_dir: Path):
    ...
    assert len(result) == 3
    assert list(result.columns) == ["confidence", "file_name"]
...
def test_load_footprints_preserves_existing_file_name_column(single_shapefile_footprints_data_dir: Path):
    result = load_footprints(single_shapefile_footprints_data_dir)
